...
function parse_binary_exp(json_object) {
    let ans = get_details(json_object.left) + ' ' + json_object.operator + ' ' + get_details(json_object.right);
    return ans;
}
function parse_unary_exp(json_object) {
    let ans = json_object.operator + '' + get_details(json_object.argument);
    return ans;
}
function parse_member_exp(json_object) {
    let ans = get_details(json_object.object) + '[' + get_details(json_object.property) + ']';
    return ans;
}
function parse_update_exp(json_object) {
    let operator = json_object.operator;
    let argument = get_details(json_object.argument);
    if(json_object.prefix)
        return operator +''+argument;
    else
        return argument +''+operator;
}
function parse_assign_exp(json_object) {
    let curr_model = new Model(json_object.loc.start.line, json_object.type, get_details(json_object.left),'',  get_details(json_object.right) );
...
function parse_return_stmt(json_object) {
    let curr_model = new Model(json_object.loc.start.line, json_object.type,'','',get_details(json_object.argument));
    list_model.push(curr_model);
}
...
function parse_if_stmt(json_object, inside_else_stmt) {
    ...
    if(inside_else_stmt) type = 'ElseIfStatement';
...
function parse_while_stmt(json_object) {
    ...
    let curr_model = new Model(json_object.loc.start.line, json_object.type,'',condition,'');
    list_model.push(curr_model);
...
function parse_variable_declarator(json_object) {
    let curr_model = new Model(json_object.loc.start.line,json_object.type, get_details(json_object.id),'',get_details(json_object.init));
    list_model.push(curr_model);
}
function parse_func_decl(json_object) {
    let curr_model = new Model(json_object.loc.start.line, json_object.type, get_details(json_object.id) ,'','');
    list_model.push(curr_model);
